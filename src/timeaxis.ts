...
export class TimeAxis {
    ...
    plotDateTimeLabels(): void {
        this.element.innerHTML = ""

        let lastPeriodIndex = 0
        for (let index = 0; index <= this.viewport.dataCursor.max; index ++) {
            let item = this.viewport.displayData[index] as OHLC
            let previousItem = this.viewport.displayData[index - 1] as OHLC
            let timeLabelText = ""
            const date = item ? (this.viewport.displayData[index] as OHLC).datetime : new Date()
            if (this.viewport.options.timeframe === Timeframe.H1) {
            ...
            (() => {
                ...
                if (index == 0 && item) {
                    ...
                    if (previousItem.datetime.getDate() !== item.datetime.getDate()) {
                        ...
                        let prevLabel = this.element.querySelector(`#period-${item.datetime.getDate()}-${item.datetime.getMonth()}`) as HTMLElement
                        if (prevLabel) {
